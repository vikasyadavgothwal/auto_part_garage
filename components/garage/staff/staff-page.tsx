...
import { Check, EllipsisVertical, Eye, Pencil, Search, Trash2, UserCheck, UserX, X } from "lucide-react"
...
export function GarageStaffPage({ access, membersPayload, rolesPayload }: { access: BusinessAccessEntry | undefined; membersPayload: MembersResponse; rolesPayload: RolesResponse }) {
  ...
  const [statusMember, setStatusMember] = useState<StaffMemberItem | null>(null)
  ...
  const showError = (errorMessage: string) => { setInviteMessage(errorMessage); toast.error(errorMessage) }
  const showMemberError = (errorMessage: string) => { setMemberMessage(errorMessage); toast.error(errorMessage) }
  ...
  const validateInvite = () => {
    ...
    if (normalizedFirstName.length > 100 || !namePattern.test(normalizedFirstName)) return "First name can contain letters, spaces, apostrophes, and hyphens only."
    if (!normalizedLastName) return "Staff last name is required."
    if (normalizedLastName.length > 100 || !namePattern.test(normalizedLastName)) return "Last name can contain letters, spaces, apostrophes, and hyphens only."
  ...
  const submitInvite = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      else { toast.success("Staff account created successfully."); closeInviteDialog(); router.refresh() }
  ...
  const submitEdit = async () => {
    ...
    if (editingFirstName.trim().length > 100 || !namePattern.test(editingFirstName.trim())) return showMemberError("First name can contain letters, spaces, apostrophes, and hyphens only.")
    if (!editingLastName.trim()) return showMemberError("Staff last name is required.")
    if (editingLastName.trim().length > 100 || !namePattern.test(editingLastName.trim())) return showMemberError("Last name can contain letters, spaces, apostrophes, and hyphens only.")
    ...
    try {
      ...
      else { toast.success("Staff roles updated successfully."); setEditingMember(null); router.refresh() }
  ...
  const submitStatusChange = async () => {
    ...
    if (!access?.businessAccount?.id) return showMemberError("Account context is not available.")
    const nextStatus = statusMember.status === "Suspended" ? "Active" : "Suspended"
    setMemberLoading(true); setMemberMessage(null)
    try {
      const response = await fetch(`/api/business/members/${encodeURIComponent(statusMember.id)}?businessAccountId=${encodeURIComponent(access.businessAccount.id)}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ status: nextStatus }) })
      const payload = await response.json().catch(() => null)
      if (!response.ok) showMemberError((payload && (payload.message ?? payload.error)) || "Unable to update staff status.")
      else { toast.success(nextStatus === "Active" ? "Staff account activated successfully." : "Staff account unactivated successfully."); setStatusMember(null); router.refresh() }
    } catch (error) { showMemberError(error instanceof Error ? error.message : "Unable to update staff status.") }
    finally { setMemberLoading(false) }
  }
  const submitDelete = async () => {
    ...
    try {
      ...
      else { toast.success("Staff deleted successfully."); setDeletingMember(null); router.refresh() }
  ...
  return (
    <div className="space-y-8">
      <div><h1 className="text-3xl font-bold">Staff</h1><p className="mt-1 text-sm text-[#9CA3AF]">Manage garage staff accounts and role assignment.</p><p className="mt-2 text-xs text-muted-foreground">Plan usage: {formatLimit(access?.businessAccount?.usage?.staff, access?.businessAccount?.limits?.staff)} staff</p></div>
      ...
      <Dialog open={isInviteOpen} onOpenChange={(open) => (open ? setIsInviteOpen(true) : closeInviteDialog())}><DialogContent className="max-h-[calc(100dvh-2rem)] max-w-2xl overflow-y-auto"><DialogHeader><DialogTitle>Create staff</DialogTitle><DialogDescription>Create a staff account, assign roles, and apply the login method allowed by this plan. Enterprise SSO can be enabled when the client identity provider is configured.</DialogDescription></DialogHeader><form onSubmit={submitInvite} noValidate className="mt-4 space-y-4"><div className="grid gap-4 md:grid-cols-2"><label className="grid gap-2"><span className="text-sm font-medium">First name <span className="text-destructive">*</span></span><Input value={firstName} onChange={(event) => setFirstName(event.target.value)} placeholder="First name" maxLength={100} required /></label><label className="grid gap-2"><span className="text-sm font-medium">Last name <span className="text-destructive">*</span></span><Input value={lastName} onChange={(event) => setLastName(event.target.value)} placeholder="Last name" maxLength={100} required /></label></div><label className="grid gap-2"><span className="text-sm font-medium">Email <span className="text-destructive">*</span></span><Input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="garage-staff@example.com" maxLength={254} required /></label><RoleSelector roles={roles} filteredRoles={filteredRoles} selectedRoles={selectedRoles} setSelectedRoles={setSelectedRoles} roleQuery={roleQuery} setRoleQuery={setRoleQuery} toggleRoleSelection={toggleRoleSelection} />{inviteMessage ? <p className="text-sm text-destructive">{inviteMessage}</p> : null}<DialogFooter><Button type="button" variant="outline" onClick={closeInviteDialog} disabled={loadingInvite}>Cancel</Button><Button type="submit" disabled={loadingInvite}>{loadingInvite ? "Creating..." : "Create account"}</Button></DialogFooter></form></DialogContent></Dialog>
      ...
      <p className="text-xs text-muted-foreground">Need more seats/roles? Open the <a href={appRoutes.plans} className="text-primary">Plans</a> page.</p>
    </div>
  )
}
